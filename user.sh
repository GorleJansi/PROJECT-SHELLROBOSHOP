...
cp "$script_dir/user.service" /etc/systemd/system/user.service >> "$logfile" 2>&1
validate $? "custom app so coping service file"

mkdir -p /app >> "$logfile" 2>&1
validate $? "creating application directory"

curl -o /tmp/user.zip https://roboshop-artifacts.s3.amazonaws.com/user-v3.zip >> "$logfile" 2>&1
validate $? "downloading artifact"

cd /app >> "$logfile" 2>&1
validate $? "change to app directory"

unzip -o /tmp/user.zip >> "$logfile" 2>&1
